---
layout: single
title: "서블릿"
categories: [mvc1]
tags: [mvc1]
toc: true
author_profile: true
---

# ☕ Spring 공부 기록

## 📘 학습 날짜
- 2025-07-21

## 📅 오늘 배운 내용

### ✨1. 서블릿 등록, 사용

- @WebServlet(name = "~" , urlPattern = "/hello")
- extends HttpServlet
- protected void service 오버라이딩
```markdown
@WebServlet(name = "helloServlet", urlPatterns = "/hello")
public class HelloServlet extends HttpServlet {
@Override
response)
protected void service(HttpServletRequest request, HttpServletResponse
throws ServletException, IOException {
System.out.println("HelloServlet.service");
System.out.println("request = " + request);
System.out.println("response = " + response);
String username = request.getParameter("username");
System.out.println("username = " + username);
response.setContentType("text/plain");
response.setCharacterEncoding("utf-8");
response.getWriter().write("hello " + username);
}
}
```
### ✨2. 서블릿 컨테이너 동작 방식

- 스프링 부트가 내장 톰캣 사용하여 웹 서버 생성
- 톰캣이 서블릿 컨테이너 역할 하며, 스프링이 서블릿 생성하고 톰캣에 등록


### ✨ 3. HttpServletRequest

- 역할 : 서블릿이 Http 요청 메시지를 파싱한 것
- 다음과 같은 요청 메시지 편리하게 조회 가능
```markdown
POST /save HTTP/1.1
Host: localhost:8080
Content-Type: application/x-www-form-urlencoded
username=kim&age=20
```
- 임시 저장소 기능 제공 
- 저장 : request.setAttribute(name,value)
- 조회 : request.getAttribute(name)
- 세션 관리 기능 : request.getSession(create: true)

### ✨ 4. HTTP 요청 데이터

- Get 쿼리 파라미터
- 메시지 바디 없이, URL의 쿼리 파라미터에 데이터를 포함해서 전달
- /url?username=kim&age=20
- Post - HTML 폼
- content-type:application/x-www-form-urlencoded
- 메시지 바디에 쿼리 파라미터 형식으로 전달
- Http 메시지 바디에 데이터를 직접 담아서 요청
- HTTP API에서 주로 사용 (JSON,XML,TEXT)
- 주로 JSON 사용 (Post,Put,Patch)

#### 4-1. GET 쿼리 파라미터

/request-param?username=park&age=20

파라미터 조회 메서드들 
```markdown
String username = request.getParameter("username"); //단일 파라미터 조회
Enumeration<String> parameterNames = request.getParameterNames(); //파라미터 이름들
모두 조회
Map<String, String[]> parameterMap = request.getParameterMap(); //파라미터를 Map으로
조회
String[] usernames = request.getParameterValues("username"); //복수 파라미터 조회
```

#### 4-2. POST HTML Form

- 주로 회원가입, 상품 주문 등에서 사용하는 방식
- content-type:application/x-www-form-urlencoded
- 메시지 바디에 쿼리 파라미터 형식으로 데이터 전달(username=park&age=20)

#### 4-3. API 메시지 바디 단순 텍스트, JSON

- 단순 텍스트 메시지 바디 : hello
- JSON : {"username" : "hello", "age" : 20}

### 5. ✨ HttpServletResponse

- 역할 : HTTP 응답토드 지정, 헤더생성, 바디 생성
- 편의 기능 제공 : Content-Type, 쿠키, Redirect

#### 5-1. 응답 데이터 단순 텍스트, HTML, JSON

- 단순 텍스트 응답 : PrintWriter writer = response.getWriter(); writer.println("ok")
- HTML 응답
```markdown
writer.println("<html>");
writer.println("<body>");
writer.println(" <div>안녕?</div>");
writer.println("</body>");
writer.println("</html>");
```

- JSON 응답
```markdown
@WebServlet(name = "responseJsonServlet", urlPatterns = "/response-json")
public class ResponseJsonServlet extends HttpServlet {
private ObjectMapper objectMapper = new ObjectMapper();
@Override
response)
protected void service(HttpServletRequest request, HttpServletResponse
throws ServletException, IOException {
//Content-Type: application/json
response.setHeader("content-type", "application/json");
response.setCharacterEncoding("utf-8");
HelloData data = new HelloData();
data.setUsername("kim");
data.setAge(20);
//{"username":"kim","age":20}
String result = objectMapper.writeValueAsString(data);
response.getWriter().write(result);
}
}
```

