...
| `docker volume rm <vol_name>`     | **특정 명명된/익명 볼륨** 삭제, **삭제 시 볼륨 안에 저장되어있던 데이터 복구 불가능**
| `docker volume prune`             | **사용되지 않는 모든 볼륨** 일괄 삭제 (주로 익명 볼륨 정리, 연결이 끊긴 익명 볼륨)
| `docker volume create <vol_name>` | **이름을 가진 볼륨 생성**
| `docker volume inpsect <vol_name>` | **볼륨 검사 (read only, 생성일자 등 확인 가능)**
