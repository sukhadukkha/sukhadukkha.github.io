...
- 컨테이너는 소스코드를 스냅샷에서 복사하는 것이 아닌, 매핑된 호스트 경로에서 복사하기 때문에 변경사항이 있더라도 컨테이너는 항상 최신 코드에 액세스 할 수 있다.
...
- 도커는 익명 볼륨을 생성할 때, 마운트 할 경로인 /app/node_modules를 확인하고, 설치된 파일들이 있는 것을 확인한다.
- 도커는 비어있는 새로운 볼륨을 마운트할 때, 그 경로의 이미지 레이어에 내용이 있으면 그 내용을 새 볼륨으로 복사하여 넣는다. 
- 즉 /app/node_modules에 있는 기존 파일들을 익명 볼륨으로 복사하고, 이 node_modules로 채워진 볼륨이 /app/node_modules에 마운트 된다.
...
- Docker Container는 기본적으로 Read-write 권한을 가지고 있다. 이는 컨테이너 내부에서 바인드 마운트 한 그 경로에 파일을 변경할 수 있다는 뜻이다.
- `docker run -d --name feedback-app -p 3000:80 --rm -v feedback:/app/feedback -v "/Users/jihopark/DevOpsStudy/Docker/data-volumes-01-starting-setup:/app:ro" -v /app/node_modules -v /app/temp feedback-node:volumes`
- :ro -> Read-Only
- `이 옵션을 통해 도커가 그 폴더나 그 하위 폴더의 Write권한을 가지지 않게 만들 수 있다. 이를 통해 컨테이너 내부에서 호스트 머신의 원본 소스 파일이 손상되지 않게 되었다.` 
- **도커는 볼륨 마운트에서 상세 경로의 우선권을 준다.** 
- 즉 저 명령어의 -v /app/node_modules, -v /app/temp, feedback:/app/feedback 이 경로들은 그대로 Read-write 권한을 가지는 것이다.
- 만약 내 호스트 머신에 node_modules 라는 폴더가 없으면 /app 밑에 호스트 머신의 경로를 바인드 마운트 :ro를 해놓았기 때문에 **write권한이 없어 자동으로 /app/node_modules 폴더를 생성해주지 못하기 때문에 사용자가 직접 빈 폴더를 만들어놔야한다. 그렇지 않으면 밑의 에러가 발생한다.**

```
Error response from daemon: failed to create task for container: failed to create shim task: OCI runtime create failed: runc create failed: unable to start container process: error during container init: error mounting "/var/lib/docker/volumes/aec4e8c47e379ee569947e15bcfa12907fbf654fe26258754641931d88157f58/_data" to rootfs at "/app/node_modules": create mountpoint for /app/node_modules mount: make mountpoint "/app/node_modules": mkdirat /var/lib/docker/rootfs/overlayfs/b3d5068a87f32f6852f2f96606b5fec496695fbb8a1706750490c046ae3efbf6/app/node_modules: read-only file system
```
---

## COPY VS Bind mount 

- `바인드 마운트로 전체 폴더를 볼륨으로 사용하는데 왜 여전히 Dockerfile의 COPY를 할까? 어짜피 덮어씌워지는 것 아닌가?`
- **Bind mount는 개발 중에 docker run 명령어로 사용하는 것이다.**
- 우리는 서버에서 docker run을 통해 바인드 마운트를 사용해서 애플리케이션을 실행하지는 않는다.
- 데이터가 유지되도록 볼륨을 사용할 수 있지만, 바인드 마운트는 사용하지 않는다.
- **그 이유는 컨테이너가 서버상의 프로덕션 상태로 실행중이면 실행되는 동안 서버의 코드에는 변경이 일어나면 안되기 때문이다.**
- `프로덕션 환경에서는 코드의 스냅샷이 필요하다.` 
---

## .dockerignore

- .dockerignore 파일을 통해 COPY 명령으로 복사해서는 안되는 폴더와 파일을 지정할 수 있다.
