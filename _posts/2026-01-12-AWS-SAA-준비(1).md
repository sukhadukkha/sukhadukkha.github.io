...
```json
{
  "Version": "2012-10-17", // 정책 문법의 버전 (가급적 최신 버전인 이 날짜를 유지해야 함)
  "Statement": [ // 실제 권한 규칙들을 담는 리스트 (여러 개의 규칙을 넣을 수 있음)
    {
      "Effect": "Allow", // 이 규칙의 효과: "허용(Allow)" 또는 "거부(Deny)"
      "Action": [ // 허용할 구체적인 행동(API 작업)들
        "s3:ListBucket", // 버킷 안에 어떤 파일이 있는지 목록을 볼 수 있는 권한
        "s3:GetObject"   // 버킷 내의 실제 파일(객체) 내용을 읽거나 다운로드할 수 있는 권한
      ],
      "Resource": [ // 위 Action이 적용될 대상(리소스)의 ARN
        "arn:aws:s3:::my-example-bucket",   // 버킷 그 자체 (ListBucket 권한을 위해 필요)
        "arn:aws:s3:::my-example-bucket/*" // 버킷 내부의 모든 파일 (GetObject 권한을 위해 필요)
      ]
    }
  ]
}
