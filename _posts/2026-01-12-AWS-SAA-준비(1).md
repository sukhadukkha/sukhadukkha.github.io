...
- IAM Policy (정책): "무엇을 할 수 있는가? (What)"
- IAM 정책은 권한을 정의한 JSON 문서다. 그 자체로는 아무런 힘이 없으며, 사용자(User), 그룹(Group), 혹은 역할(Role)에 부착(Attach)되어야만 효과가 발생한다.
...
- IAM Role (역할): "누구든 잠시 빌려 쓸 수 있는 가면 (Who)"
